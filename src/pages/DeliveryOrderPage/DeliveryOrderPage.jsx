...
import PathaoOrderTable from "../../components/Order/PathaoOrderTable";
...
const DeliveryOrderPage = () => {
  ...
  const {
    ...
    queryKey: [
      `/api/v1/order/dashboard?order_status=delivered&page=${page}&limit=${limit}&searchTerm=${searchTerm}`,
    ],
    queryFn: async () => {
      const res = await fetch(
        `${BASE_URL}/order/dashboard?order_status=delivered&page=${page}&limit=${limit}&searchTerm=${searchTerm}`,
        {
          credentials: "include",
        }
      );
