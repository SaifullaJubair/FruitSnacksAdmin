  ...
  (
    ...
    return (
      ...
      `}</style>
        <div
          ref={ref}
          className="receipt-print hidden print:block font-mono text-sm text-black bg-white p-4 w-80"
        >
          <div className="text-center mb-3">
            <p className="text-base font-bold">{shopName || "FruitSnacks"}</p>
            {shopPhone && <p className="text-xs">{shopPhone}</p>}
            <p className="text-xs mt-1">POS Receipt</p>
            <p className="text-xs">{now}</p>
            {invoiceId && (
              <p className="text-xs font-semibold mt-1">Invoice: {invoiceId}</p>
            )}
          </div>
